...
from src.auth_db import DB_FILE, init_db, create_user, authenticate_user, get_user_by_id, log_guest_crawl, get_guest_crawls_last_24h, verify_user, set_user_tier, create_verification_token, verify_token, get_user_by_email
...
# If not set in environment, try to persist it in the data directory so sessions survive restarts.
SECRET_KEY = os.environ.get('SECRET_KEY')
if not SECRET_KEY:
    db_dir = os.path.dirname(DB_FILE)
    secret_key_file = os.path.join(db_dir, 'secret.key')
    
    if os.path.exists(secret_key_file):
        try:
            with open(secret_key_file, 'r') as f:
                SECRET_KEY = f.read().strip()
            print(f"🔑 Loaded persistent SECRET_KEY from: {secret_key_file}", flush=True)
        except Exception as e:
            print(f"⚠️ Error reading persistent secret key file: {e}", flush=True)
    ...
    if not SECRET_KEY:
        ...
        try:
            ...
            with open(secret_key_file, 'w') as f:
                f.write(SECRET_KEY)
            print(f"🔑 Generated and saved persistent SECRET_KEY to: {secret_key_file}", flush=True)
        except Exception as e:
            print(f"⚠️ Warning: Could not save persistent secret key to {secret_key_file} ({e}). "
                  "Using an ephemeral key. Sessions will not persist across restarts.", flush=True)
